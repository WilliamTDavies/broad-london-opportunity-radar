...
Verified 12 August 2026. A configured employer is a monitoring target, not a role whitelist. Thirty-three sources are enabled: three curated official-ATS snapshots, one live official Workday search/detail feed, eleven employer-agnostic London discovery/government feeds and eighteen official page-change monitors. The checked-in snapshot contains 16 verified roles and 982 possible roles across 570 employers. The current health snapshot represents 7,359 listing appearances, including the earlier complete captures of 1,235 CharityJob, 1,106 NHS Jobs, 433 jobs.ac.uk, 151 W4MP, 69 Higherin and 79 targetjobs records plus a 4,182-appearance/1,475-unique DWP query-matrix capture, 86 Prospects cards and one Legal Cheek card. Adzuna and Reed are configured but contribute zero until their API secrets are supplied. The remaining 203 registry entries are disabled until a safe endpoint or curated official record is verified. Coverage is materially broader, but it is not a guarantee of every job on the web.
...
|---|---|---|---|
| W4MP Jobs | Trusted sector board | <https://www.w4mpjobs.org/searchjobs.aspx?search=alljobs> | Enabled. The ASP.NET pager adapter scans all eight current result pages, preserves the named organisation/office as employer and W4MP as publisher, and passes all 151 listings to central classification; 56 remain possible. |
...
| CharityJob London | Discovery-only board | <https://www.charityjob.co.uk/jobs/in-london> | Enabled. The rate-limited adapter scans all 83 pages and verifies the 1,235 advertised-result count. It deduplicates responsive copies, handles cards without logos, and currently contributes 452 possible roles. |
| NHS Jobs London | Official government portal | <https://www.jobs.nhs.uk/candidate/search/results?location=London> | Enabled. The date-sorted adapter requests 100 results per page, scans all 12 pages and verifies the 1,106 advertised-result count. Specialist clinical roles are excluded; 112 plausibly accessible administrative, analytical and programme roles remain possible. |
| jobs.ac.uk London | Discovery-only board | <https://www.jobs.ac.uk/search/location/london> | Enabled. The facet adapter scans all 18 London result pages and verifies the 433 advertised-result count. Senior academic and specialist research/clinical roles are excluded; 110 plausibly accessible roles remain possible. |
...
| Adzuna London | Discovery-only API | <https://developer.adzuna.com/docs/search> | Configured and enabled, but locally inactive until `ADZUNA_APP_ID` and `ADZUNA_APP_KEY` are supplied. The documented API uses the same 30-query matrix, a 30-day window, pagination and ID deduplication. |
